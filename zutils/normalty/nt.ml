...
let to_smtty t =
  let aux = function
    ...
    | _ ->
        ...
        in
        _die_with [%here] "not a basic type"
  in
  aux t
...
let get_argty = function Ty_arrow (t1, _) -> t1 | _ -> _die_with [%here] "?"
let get_retty = function Ty_arrow (_, t2) -> t2 | _ -> _die_with [%here] "?"
            ...
            let cs =
              List.fold_left
                (fun res (x, t) ->
                  ...
                  | None -> _die_with [%here] (spf "connot find feild %s" x)
                  | Some t' -> (t', t) :: res)
                cs l2
            in
            aux m cs
