...
machine Proposer {
    ...
    state Loop {
        ...
        on syn_eAcceptRsp do (input: tsyn_eAcceptRsp) {
            RealSend(leaner, syn_eLearn, (controller = input.controller, dst = leaner, va = input.va));
            goto Loop;
        }
    }
    fun unreadlizablePrepareReq (input: tsyn_ePrepareReq) {
        ...
        if ($ && $) {
            RealSend(input.dst, syn_ePrepareReq, input);
        }
    }
    fun unreadlizableAcceptReq (input: tsyn_eAcceptReq) {
        ...
        pair.p = input.proposer;
        if ($ && $) {
            RealSend(input.dst, syn_eAcceptReq, input);
        }
    }
}
...
machine Acceptor {
    ...
    fun unreadlizablePrepareRsp (input: tsyn_ePrepareRsp) {
        ...
        if ($) {
            RealSend(input.dst, syn_ePrepareRsp, input);
        }
    }
    fun unreadlizableAcceptRsp (input: tsyn_eAcceptRsp) {
        ...
        pair.p = input.proposer;
        if ($ || $) {
            RealSend(input.dst, syn_eAcceptRsp, input);
        }
    }
}
