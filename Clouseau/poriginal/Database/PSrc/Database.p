machine Database {
    ...
    state Wait {
        ...
        on syn_readReq do (input: tsyn_readReq) {
            if(is_init) {
                RealSend(input.controller, syn_readRsp, (controller = input.controller, dst = input.controller, va = store, st = true));
            } else {
                RealSend(input.controller, syn_readRsp, (controller = input.controller, dst = input.controller, va = store, st = false));
            }
            goto Wait;
        }
    }
    fun do_send_writeRsp(input: tsyn_writeRsp) {
        RealSend(input.controller, syn_writeRsp, input); 
    }
}