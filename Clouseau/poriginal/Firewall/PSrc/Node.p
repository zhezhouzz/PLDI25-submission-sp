machine Internal {
    ...
    start state Init {
        ...
        on syn_eStart do (input: tsyn_eStart) {
            send fw, syn_eInternalReq, (controller = input.controller, dst = fw, node = input.node);
        }
    ...
    state Loop {
        ...
        on syn_eExternalReq do (input: tsyn_eExternalReq) {
            if (lastNode == input.node) {
                send internal, syn_eExternalRsp, (controller = input.controller, dst = internal, node = input.node, stat = true);
            } else {
                send internal, syn_eExternalRsp, (controller = input.controller, dst = internal, node = input.node, stat = false);   
            }
            goto Loop;
        }
    }
...
machine External {
    ...
    start state Init {
        ...
        on syn_eForwardReq do (input: tsyn_eForwardReq) {
            send fw, syn_eExternalReq, (controller = input.controller, dst = fw, node = input.node);
        }
    }
}