...
machine CoffeeMakerControlPanel
{
  ...
  state CoffeeMakerReady {
    entry {
      ...
      RealSend(currentUser, syn_eCoffeeMakerReady, (controller = currentUser, dst = currentUser));
    }
  ...
  state CoffeeMakerRunEspresso {
    ...
    on eEspressoCompleted goto CoffeeMakerReady with { RealSend(currentUser, syn_eEspressoCompleted, (controller = currentUser, dst = currentUser)); }
  ...
  state EncounteredError {
    entry {
      ...
      RealSend(currentUser, syn_eCoffeeMakerError, (controller = currentUser, dst = currentUser, st = coffeeMakerState));
    }
  ...
  fun BeginHeatingCoffeeMaker() {
    ...
    RealSend(coffeeMaker, syn_eWarmUpReq, (controller = currentUser, dst = coffeeMaker));
  }
  ...
  fun GrindBeans() {
    ...
    RealSend(coffeeMaker, syn_eGrindBeansReq, (controller = currentUser, dst = coffeeMaker));
  }

  fun StartEspresso() {
    // send an event to maker to start espresso
    RealSend(coffeeMaker, syn_eStartEspressoReq, (controller = currentUser, dst = coffeeMaker));
  }
}
