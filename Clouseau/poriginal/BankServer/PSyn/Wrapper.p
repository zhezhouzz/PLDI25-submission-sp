fun send_eWithDrawReq (src: machine, dest: machine, input: (rId: rid, accountId: aid, amount: int)) {
  RealSend(dest, syn_eWithDrawReq, (controller = src, dst = dest, rId = input.rId, accountId = input.accountId, amount = input.amount));
}

fun send_eInitAccount (src: machine, dest: machine, input: (accountId: aid, balance: int)) {
  RealSend(dest, syn_eInitAccount, (controller = src, dst = dest, accountId = input.accountId, balance = input.balance));
}
